...
router.get("/vnpay_return", async function (req, res, next) {
  ...
  // Lấy giá trị vnp_OrderInfo từ query string
  let orderInfo = vnp_Params["vnp_OrderInfo"];

  // Tách orderId từ vnp_OrderInfo
  // Ví dụ nếu orderInfo có định dạng như "Thanh toan cho ma GD:OrderIdXYZ"
  let orderId = orderInfo.split(":")[1]; // Lấy phần sau dấu ":"

  console.log("Order ID: ", orderId); // Log ra orderId đã tách
  ...
  if (secureHash === signed) {
    // Xử lý tiếp theo, ví dụ: lưu orderId vào DB hoặc cập nhật trạng thái đơn hàng
    // res.status(200).json({
    //   success: true,
    //   code: vnp_Params["vnp_ResponseCode"],
    //   orderId: orderId, // Trả lại orderId trong response nếu cần thiết
    // });
